		...
		<div id="list-run" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="date" title="${message(code: 'run.date.label', default: 'Date')}" />
				...
				<g:each in="${runInstanceList}" status="i" var="runInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${runInstance.id}">${fieldValue(bean: runInstance, field: "date")}</g:link></td>
			...
			<div class="pagination">
				<g:paginate total="${runInstanceCount ?: 0}" />
			</div>
		</div>
	</body>
</html>
